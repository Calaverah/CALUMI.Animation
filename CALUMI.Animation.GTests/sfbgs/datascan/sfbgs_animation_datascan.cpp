...
using namespace CALUMI;
	...
	{
		std::filesystem::path animPath = SFBGS_Data::get().dataPath.string() + "/meshes/architecture/city/akila/animated/doors/ak_ext_bld_walla_doora_01/animations/open.af";
		...
		SFBGS::AnimationBlock animBlock = anim.getAnimationBlocks().at(1);

		Math::Quaternion rotation = SFBGS::GetUniversalRotation(animBlock.RotationPrefixEntries().at(animBlock.getRotationPrefixCount() - 1), animBlock.RotationEntries().at(animBlock.getRotationCount() - 1));
	...
	{
		std::filesystem::path animPath = SFBGS_Data::get().dataPath.string() + "/meshes/architecture/city/newatlantis/lodge/na_lodgedoor02/animations/open.af";
		ASSERT_TRUE(std::filesystem::exists(animPath)) << "FILE DOES NOT EXIST" << std::endl;
		ASSERT_TRUE(std::filesystem::is_regular_file(animPath)) << "FILE IS NOT REGULAR FILE" << std::endl;
		...
		SFBGS::AnimationBlock animBlockLeft = anim.getAnimationBlocks().at(1);
		SFBGS::AnimationBlock animBlockRight = anim.getAnimationBlocks().at(4);

		Math::Quaternion rotationLeft = SFBGS::GetUniversalRotation(animBlockLeft.RotationPrefixEntries().at(animBlockLeft.getRotationPrefixCount() - 1), animBlockLeft.RotationEntries().at(animBlockLeft.getRotationCount() - 1));
		Math::Quaternion expectedLeft(Math::Vector3(0.0, 0.0, 1.0), Math::ToRadians(90.0f));
		Math::Quaternion rotationRight = SFBGS::GetUniversalRotation(animBlockRight.RotationPrefixEntries().at(animBlockRight.getRotationPrefixCount() - 1), animBlockRight.RotationEntries().at(animBlockRight.getRotationCount() - 1));
