...
namespace CALUMI {
	namespace Utilities {
		...
		public:
			 ...
			 * @return 
			 */
			CALUMIANIMATION_API static HashRegistry& getInstance();
			 ...
			 */
			CALUMIANIMATION_API const char* getKnownHashString(uint32_t hash);
			/**
			 * @brief A simple way to see if a known input is paired to this hash key
			 * @param hash 
			 * @return 
			 */
			CALUMIANIMATION_API bool isKnownHash(uint32_t hash) const;
			/**
			 * @brief Will add this string to the registry
			 * @param string String to add
			 * @return Hash key generated from string input
			 */
			CALUMIANIMATION_API uint32_t registerHash(const char* string);
			/**
